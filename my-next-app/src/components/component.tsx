...
export function Component() {
  ...
  const handleToggleQA = async () => {
    setLoading(true);
    try {
      const url = new URL(`http://localhost:8080/qaboard/ada/`);
      ...
      if (response.ok) {
        ...
        setQnaData(data.body.result);
        // 초기 토글 상태 설정: 모든 항목을 false (숨김)으로 설정
        const initialState = data.body.result.reduce((acc, item, index) => {
          acc[index] = false;
          return acc;
        }, {});
            ...
            <CollapsibleTrigger asChild>
              <div className="flex items-center space-x-4 cursor-pointer" onClick={() => toggleItem(index)}>
                <div className="flex-1">
                  <h3 className="text-xl font-semibold">{qna.question}</h3>
                  <p className="text-sm text-black dark:text-black">{qna.questionType}</p>
                </div>
              </div>
            </CollapsibleTrigger>
            {openItems[index] && (
